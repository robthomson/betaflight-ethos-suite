...
def resolve_i18n_tags_in_place(out_dir, lang="en"):
    ...
    json_path = os.path.join(out_dir, "i18n", f"{lang}.json")  # because scripts/bfsuite/** got copied already
    if not os.path.isfile(json_path):
        # Fallback for local build before copy, if needed:
        json_path = os.path.join(config['git_src'], "scripts", "bfsuite", "i18n", f"{lang}.json")
    if not os.path.isfile(json_path):
...
def main():
    ...
    p.add_argument('--lang', default=os.environ.get("bfsuite_LANG", "en"),
                   help='Locale to resolve (e.g. en, de, fr). Defaults to env bfsuite_LANG or "en".')
