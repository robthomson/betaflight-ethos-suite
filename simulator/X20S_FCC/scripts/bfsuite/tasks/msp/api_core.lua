...
local mspHelper = bfsuite.tasks.msp.mspHelper
local utils = bfsuite.utils
local callback = bfsuite.tasks.callback
...
local function parseMSPChunk(buf, structure, state)
    ...
    while state.index <= #structure and processedFields < 5 do
        ...
        if field.apiVersion and bfsuite.utils.apiVersionCompare("<", field.apiVersion) then goto continue end
...
function core.calculateMinBytes(structure)
    ...
    for _, param in ipairs(structure) do
        ...
        if not param.apiVersion or bfsuite.utils.apiVersionCompare(">=", param.apiVersion) then insert_param = true end
...
function core.buildWritePayload(apiname, payload, api_structure, noDelta)
    if not bfsuite.app.Page then
        utils.log("[buildWritePayload] No page context available", "info")
        return nil
    end

    local positionmap = bfsuite.tasks.msp.api.apidata.positionmap[apiname]
    local receivedBytes = bfsuite.tasks.msp.api.apidata.receivedBytes[apiname]
    local receivedBytesCount = bfsuite.tasks.msp.api.apidata.receivedBytesCount[apiname]
...
function core.buildDeltaPayload(apiname, payload, api_structure, positionmap, receivedBytes, receivedBytesCount)
    ...
    for idx, formField in ipairs(bfsuite.app.formFields) do
        local pageField = bfsuite.app.Page.apidata.formdata.fields[idx]
    ...
    local actual_fields = {}
    if bfsuite.app.Page and bfsuite.app.Page.apidata then
        for _, field in ipairs(bfsuite.app.Page.apidata.formdata.fields) do

            if field.api and not field.apikey then
                local mspapi, apikey = string.match(field.api, "([^:]+):(.+)")

                for i, api in ipairs(bfsuite.app.Page.apidata.api) do
                    if api == mspapi then
                        mspapi = i
                        break
                    end
                ...
                field.apikey = apikey
                field.mspapi = mspapi
                bfsuite.utils.log("[buildDeltaPayload] Converted api field '" .. field.api .. "' to mspapi=" .. tostring(mspapi) .. " apikey=" .. tostring(apikey), "info")
            end
            if not field.apikey then
                bfsuite.utils.log("[buildDeltaPayload] Missing apikey for field: " .. tostring(field.api), "info")
            else
...
function core.buildFullPayload(apiname, payload, api_structure)
    ...
    local actual_fields = {}
    if bfsuite.app.Page and bfsuite.app.Page.apidata then for _, field in ipairs(bfsuite.app.Page.apidata.formdata.fields) do actual_fields[field.apikey] = field end end
    ...
    for _, field_def in ipairs(api_structure) do
        ...
        if not actual_field and field_def.decimals then scale = scale / bfsuite.app.utils.decimalInc(field_def.decimals) end
        value = math.floor(value * scale + 0.5)
...
function core.prepareStructureData(structure)
    ...
    for _, param in ipairs(structure) do
        if param.apiVersion and bfsuite.utils.apiVersionCompare("<", param.apiVersion) then goto continue end
