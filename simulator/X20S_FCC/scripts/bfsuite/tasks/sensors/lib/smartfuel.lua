...
local telemetry
local lastMode = bfsuite.flightmode.current or "preflight"
...
local function smartFuelCalc()
    ...
    if currentMode ~= lastMode then
        bfsuite.utils.log("Flight mode changed – resetting voltage & fuel state", "info")
    ...
    local isDisarmed = (bfsuite and bfsuite.session and bfsuite.session.isArmed == false)
    local isPreflight = (bfsuite and bfsuite.flightmode and bfsuite.flightmode.current == "preflight")
