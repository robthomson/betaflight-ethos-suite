...
local utils = bfsuite.widgets.dashboard.utils
...
local function getUserVoltageOverride(which)
    local prefs = bfsuite.session and bfsuite.session.modelPreferences
...
local function getThemeValue(key)
    if bfsuite and bfsuite.session and bfsuite.session.modelPreferences and bfsuite.session.modelPreferences[theme_section] then
        local val = bfsuite.session.modelPreferences[theme_section][key]
        val = tonumber(val)
...
local function buildBoxes(W)
    ...
    return {

        {
            ...
            min = function()
                local cfg = bfsuite.session.batteryConfig
...
local function boxes()
    local config = bfsuite and bfsuite.session and bfsuite.session.modelPreferences and bfsuite.session.modelPreferences[theme_section]
