...
local baseDir = bfsuite.config.baseDir
local preferences = bfsuite.config.preferences
...
dashboard.flightmode = bfsuite.flightmode.current or "preflight"
...
dashboard._minPaintInterval = 0.025
...
function dashboard.computeOverlayMessage()
    ...
    if dashboard.themeFallbackUsed and dashboard.themeFallbackUsed[state] and (os.clock() - (dashboard.themeFallbackTime and dashboard.themeFallbackTime[state] or 0)) < 10 then return "Your theme did not load correctly. Falling back to default theme." end

    if not tasks.active() then return "BG TASK" end

    if bfsuite.session.apiVersion and bfsuite.session.rfVersion and not bfsuite.session.isConnectedLow and state ~= "postflight" then
    ...
    if not bfsuite.session.isConnectedHigh and state ~= "postflight" then return "CONNECTING" end
...
function dashboard.renderLayout(widget, config)
    ...
    if layout.showgrid or bfsuite.preferences.developer.overlaygrid then
    ...
    if layout.showstats or bfsuite.preferences.developer.overlaystats then
        local headerOffset = (isFullScreen and headerLayout and headerLayout.height) or 0

        local cpuUsage = (bfsuite.performance and bfsuite.performance.cpuload) or 0
        local loopMs = (bfsuite.performance and bfsuite.performance.loop_ms) or 0
        local budgetMs = (bfsuite.performance and bfsuite.performance.budget_ms) or 50
        local tickMs = (bfsuite.performance and bfsuite.performance.tick_ms)
        local headroomPct = math.max(0, 100 - (cpuUsage or 0))

        local ramFreeKB = (bfsuite.performance and bfsuite.performance.luaRamKB) or 0
        local ramUsedGC_KB = (bfsuite.performance and bfsuite.performance.usedram) or 0
        local sysRamFreeKB = (bfsuite.performance and bfsuite.performance.ramKB) or 0
        local bitmapRamFreeKB = (bfsuite.performance and bfsuite.performance.luaBitmapsRamKB) or 0
        local mainStackKB = (bfsuite.performance and bfsuite.performance.mainStackKB) or 0
        ...
        local function fmtPct(n) return bfsuite.utils.round(n or 0, 0) end
        ...
        local schedRows = {{"LOAD", fmtPct(cpuUsage), "%"}, {"LOAD (100ms window)", fmtPct(bfsuite.performance.cpuload_window100 or 0), "%"}, {"HEADROOM", fmtPct(headroomPct), "%"}, {"LOOP / BUDGET", fmtMS(loopMs) .. " / " .. fmtMS(budgetMs), "ms"}}
...
local function getThemeForState(state)
    local prefs = bfsuite.session.modelPreferences and bfsuite.session.modelPreferences.dashboard
    local fallback = bfsuite.preferences.dashboard
...
local function getThemeForState(state)
    local modelPrefs = bfsuite.session.modelPreferences and bfsuite.session.modelPreferences.dashboard
    local userPrefs = bfsuite.preferences.dashboard
    local val = nil
    if modelPrefs then
...
function dashboard.create()
    ...
    if not dashboard.utils then dashboard.utils = assert(compile("SCRIPTS:/" .. bfsuite.config.baseDir .. "/widgets/dashboard/lib/utils.lua"))() end
    if not dashboard.loaders then dashboard.loaders = assert(compile("SCRIPTS:/" .. bfsuite.config.baseDir .. "/widgets/dashboard/lib/loaders.lua"))() end

    os.mkdir("SCRIPTS:/" .. bfsuite.config.preferences .. "/dashboard/")
...
function dashboard.event(widget, category, value, x, y)
    ...
    if state == "postflight" and category == EVT_KEY and value == 131 then
        bfsuite.widgets.dashboard.flightmode = "preflight"
...
function dashboard.wakeup(widget)
    ...
    if bfsuite.session and bfsuite.session.mspBusy and not (bfsuite.session and bfsuite.session.isConnected) then return end

    local now = os.clock()
    local visible = lcd.isVisible()
    local admin = bfsuite.app and bfsuite.app.guiIsRunning
    ...
    objectProfiler = bfsuite.preferences and bfsuite.preferences.developer and bfsuite.preferences.developer.logobjprof
    ...
    if firstWakeupCustomTheme and bfsuite.session.mcu_id and bfsuite.session.modelPreferences and bfsuite.session.modelPreferences.dashboard then

        local modelPrefs = bfsuite.session.modelPreferences.dashboard
        local currentPrefs = bfsuite.preferences.dashboard
    ...
    local currentFlightMode = bfsuite.flightmode.current or "preflight"
                ...
                if utils.dir_exists(basePath, folder) then
                    ...
                    if chunk then
                        ...
                        if ok and initTable and type(initTable.name) == "string" then
                            if not initTable.developer or bfsuite.preferences.developer.devtools == true then
                                num = num + 1
...
function dashboard.getPreference(key)
    if not bfsuite.session.modelPreferences or not dashboard.currentWidgetPath then return nil end

    if not bfsuite.app.guiIsRunning then
        return bfsuite.ini.getvalue(bfsuite.session.modelPreferences, dashboard.currentWidgetPath, key)
    else
        return bfsuite.ini.getvalue(bfsuite.session.modelPreferences, bfsuite.app.dashboardEditingTheme, key)
    end
end

function dashboard.savePreference(key, value)
    if not bfsuite.session.modelPreferences or not bfsuite.session.modelPreferencesFile or not dashboard.currentWidgetPath then return false end
    if not bfsuite.app.guiIsRunning then
        bfsuite.ini.setvalue(bfsuite.session.modelPreferences, dashboard.currentWidgetPath, key, value)
        return bfsuite.ini.save_ini_file(bfsuite.session.modelPreferencesFile, bfsuite.session.modelPreferences)
    else
        bfsuite.ini.setvalue(bfsuite.session.modelPreferences, bfsuite.app.dashboardEditingTheme, key, value)
        return bfsuite.ini.save_ini_file(bfsuite.session.modelPreferencesFile, bfsuite.session.modelPreferences)
    end
end
...
function dashboard.menu(widget) return {{"Reset flight", dashboard.resetFlightModeAsk}} end
