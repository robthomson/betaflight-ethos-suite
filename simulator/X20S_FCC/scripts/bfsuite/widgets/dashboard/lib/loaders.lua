...
local function drawLogoImage(cx, cy, w, h)
    local imageName = "SCRIPTS:/" .. bfsuite.config.baseDir .. "/widgets/dashboard/gfx/logo.png"
    local bmp = bfsuite.utils.loadImage(imageName)
    if bmp then
...
local function renderOverlayText(dashboard, cx, cy, innerR, fg)
    local message = dashboard._overlay_text or "betaflight"
