...
function app.paint()
    ...
    if app.ui and app.ui.adminStatsOverlay then if not bfsuite.session.mspBusy then app.ui.adminStatsOverlay() end end
...
function app.wakeup()
    ...
    if bfsuite.preferences and bfsuite.preferences.developer and bfsuite.preferences.developer.overlaystatsadmin then if not bfsuite.session.mspBusy then lcd.invalidate() end end
end
...
function app.event(widget, category, value, x, y)
    ...
    if bfsuite.app.lastMenu ~= nil and category == 3 and value == 0 then
        app.ui.openMainMenu()
        return true
    end
...
function app.close()
    bfsuite.utils.reportMemoryUsage("app.close", "start")

    local userpref_file = "SCRIPTS:/" .. bfsuite.config.preferences .. "/preferences.ini"
    bfsuite.ini.save_ini_file(userpref_file, bfsuite.preferences)
