...
function app.getRSSI()
    --if system:getVersion().simulation == true or bfsuite.preferences.skipRssiSensorCheck == true or app.offlineMode == true then return 100 end
    if app.offlineMode == true then return 100 end


    if bfsuite.session.telemetryState then
        return 100
    else
        return 0
    end
...
function app.resetState()
    ...
    bfsuite.app.triggers.profileswitchLast = nil
    bfsuite.session.activeProfileLast = nil
    bfsuite.session.activeProfile = nil
    bfsuite.session.activeRateProfile = nil
    bfsuite.session.activeRateProfileLast = nil
    bfsuite.session.activeProfile = nil
    bfsuite.session.activeRateTable = nil
...
local function rebootFc()
    ...
    bfsuite.tasks.msp.mspQueue:add({
...
local mspEepromWrite = {
    ...
    errorHandler = function(self)
        app.triggers.closeSave = true
        app.audio.playSaveArmed = true
        if bfsuite.preferences.saveWhenArmedWarning == true then app.triggers.showSaveArmedWarning = true end
    end,
    simulatorResponse = {}
}
...
function app.settingsSaved()
    ...
    if app.Page and app.Page.eepromWrite then
        ...
        if app.pageState ~= app.pageStatus.eepromWrite then
            ...
            bfsuite.tasks.msp.mspQueue:add(mspEepromWrite)
        end
    ...
    collectgarbage()
    bfsuite.utils.reportMemoryUsage("app.settingsSaved")
end
...
local function saveSettings()
    ...
    bfsuite.utils.log("Saving data", "debug")

    local mspapi = bfsuite.app.Page.mspapi
    ...
    for apiID, apiNAME in ipairs(apiList) do
        bfsuite.utils.log("Saving data for API: " .. apiNAME, "debug")

        local payloadData = values[apiNAME]
        local payloadStructure = mspapi.structure[apiNAME]

        -- Initialise the API
        local API = bfsuite.tasks.msp.api.load(apiNAME)
        ...
        API.setCompleteHandler(function(self, buf)
            ...
            bfsuite.utils.log("API " .. apiNAME .. " write complete", "debug")

            -- Check if this is the last completed request
            if completedRequests == totalRequests then
                bfsuite.utils.log("All API requests have been completed!", "debug")
        ...
        -- Send the payload
        for i, v in pairs(payloadData) do
            bfsuite.utils.log("Set value for " .. i .. " to " .. v, "debug")
            API.setValue(i, v)
        end
...
--[[
    ...
    1. Ensures that `app.Page.mspapi.formdata`, `app.Page.mspapi.api`, and `bfsuite.app.Page.fields` exist.
...
function app.mspApiUpdateFormAttributes(values, structure)
    ...
    if not (app.Page.mspapi.formdata and app.Page.mspapi.api and bfsuite.app.Page.fields) then
        bfsuite.utils.log("app.Page.mspapi.formdata or its components are nil", "debug")
        return
    end
    ...
    for i, f in ipairs(fields) do
        ...
        local formField = bfsuite.app.formFields[i]
        ...
        if type(formField) == 'userdata' then
            ...
            if f.api then
                bfsuite.utils.log("API field found: " .. f.api, "debug")
            ...
            if mspapiID  == nil or mspapiID  == nil then 
                bfsuite.utils.log("API field missing mspapi or apikey", "debug")
            else        
                for _, v in ipairs(targetStructure) do
                    ...
                    if v.field == apikey and mspapiID == f.mspapi then
                        ...
                        local help_return = bfsuite.i18n.get(help_target)
                        ...
                        bfsuite.app.ui.injectApiAttributes(formField, f, v)

                        local scale = f.scale or 1
                        if values and values[mspapiNAME] and values[mspapiNAME][apikey] then
                            bfsuite.app.Page.fields[i].value = values[mspapiNAME][apikey] / scale
                        end

                        if values[mspapiNAME][apikey] == nil then
                            bfsuite.utils.log("API field value is nil: " .. mspapiNAME .. " " .. apikey, "info")
            ...
            bfsuite.utils.log("Form field skipped; not valid for this api version?", "debug")    
        end    
    end
    collectgarbage()
    bfsuite.utils.reportMemoryUsage("app.mspApiUpdateFormAttributes")

    -- set focus back to menu
    bfsuite.app.formNavigationFields['menu']:focus(true)
end
...
--[[
    ...
    Note: The function uses bfsuite.utils.log for logging and bfsuite.tasks.msp.api.load
...
local function requestPage()
    ...
    if not app.Page.mspapi.api and not app.Page.mspapi.formdata then
        bfsuite.utils.log("app.Page.mspapi.api did not pass consistancy checks", "debug")
        return
    end

    if not bfsuite.app.Page.mspapi.apiState then
        bfsuite.app.Page.mspapi.apiState = {
            currentIndex = 1,
            isProcessing = false
        }
    end    
    ...
    local state = bfsuite.app.Page.mspapi.apiState  -- Reference persistent state

    -- Prevent duplicate execution if already running
    if state.isProcessing then
        bfsuite.utils.log("requestPage is already running, skipping duplicate call.", "debug")
        return
    end
    state.isProcessing = true  -- Set processing flag

    if not bfsuite.app.Page.mspapi.values then
        bfsuite.utils.log("requestPage Initialize values on first run", "debug")
        bfsuite.app.Page.mspapi.values = {}  -- Initialize if first run
        bfsuite.app.Page.mspapi.structure = {}  -- Initialize if first run
        bfsuite.app.Page.mspapi.receivedBytesCount = {}  -- Initialize if first run
        bfsuite.app.Page.mspapi.receivedBytes = {}  -- Initialize if first run
        bfsuite.app.Page.mspapi.positionmap = {}  -- Initialize if first run
    end
...
local function checkForUnresolvedTimeouts()
    ...
    for apiKey, retries in pairs(app.Page.mspapi.retryCount or {}) do
        if retries >= 3 then
            ...
            bfsuite.utils.log("[ALERT] API " .. apiKey .. " failed after 3 timeouts.", "info")
        end
    end
    ...
    if hasUnresolvedTimeouts then
        ...
        bfsuite.app.ui.disableAllNavigationFields()
        bfsuite.app.ui.enableNavigationField('menu')
...
local function processNextAPI()
    ...
    if not app or not app.Page or not app.Page.mspapi then
        bfsuite.utils.log("App is closing. Stopping processNextAPI.", "debug")
        return
    end
    ...
    if not apiKey then
        bfsuite.utils.log("API key is missing for index " .. tostring(state.currentIndex), "warning")
        state.currentIndex = state.currentIndex + 1
        bfsuite.tasks.callbackInSeconds(0.5, processNextAPI)
        return
    end

    local API = bfsuite.tasks.msp.api.load(v)
    ...
    -- **Log API Start**
    bfsuite.utils.log("[PROCESS] API: " .. apiKey .. " (Attempt " .. (retryCount + 1) .. ")", "debug")
    ...
    local function handleTimeout()
        ...
        if not app or not app.Page or not app.Page.mspapi then
            bfsuite.utils.log("App is closing. Timeout handling skipped.", "debug")
            return
        end
        ...
        if retryCount < 3 then  
            bfsuite.utils.log("[TIMEOUT] API: " .. apiKey .. " (Retry " .. retryCount .. ")", "warning")
            bfsuite.tasks.callbackInSeconds(0.5, processNextAPI)  
        else
            bfsuite.utils.log("[TIMEOUT FAIL] API: " .. apiKey .. " failed after 3 attempts. Skipping.", "error")
            state.currentIndex = state.currentIndex + 1
            bfsuite.tasks.callbackInSeconds(0.5, processNextAPI)  
        end
    end
    ...
    bfsuite.tasks.callbackInSeconds(2, handleTimeout)
    ...
    API.setCompleteHandler(function(self, buf)
        ...
        -- **Exit safely if app is closed**
        if not app or not app.Page or not app.Page.mspapi then
            bfsuite.utils.log("App is closing. Skipping API success handling.", "debug")
            return
        end

        -- **Log API Success**
        bfsuite.utils.log("[SUCCESS] API: " .. apiKey .. " completed successfully.", "debug")
    ...
    API.setErrorHandler(function(self, err)
        ...
        -- **Exit safely if app is closed**
        if not app or not app.Page or not app.Page.mspapi then
            bfsuite.utils.log("App is closing. Skipping API error handling.", "debug")
            return
        end
        ...
        if retryCount < 3 then  
            bfsuite.utils.log("[ERROR] API: " .. apiKey .. " failed (Retry " .. retryCount .. "): " .. tostring(err), "warning")
            bfsuite.tasks.callbackInSeconds(0.5, processNextAPI)  
        else
            bfsuite.utils.log("[ERROR FAIL] API: " .. apiKey .. " failed after 3 attempts. Skipping.", "error")
            state.currentIndex = state.currentIndex + 1
            bfsuite.tasks.callbackInSeconds(0.5, processNextAPI)  
        end
    end)
...
function app.updateTelemetryState()
    ...
    if system:getVersion().simulation ~= true then
        if not bfsuite.session.telemetrySensor then
...
function app.wakeupUI()
    ...
    if app.triggers.exitAPP == true then
        ...
        form.invalidate()
        system.exit()
        bfsuite.utils.reportMemoryUsage("Exit App")
        return
    end
    ...
    if app.triggers.closeProgressLoader == true  then
        ...
        if app.dialogs.progressCounter >= 101 and bfsuite.tasks.msp.mspQueue:isProcessed() then
            app.dialogs.progressWatchDog = nil
            app.dialogs.progressDisplay = false
    ...
    if app.triggers.closeSave == true then
        ...
        if app.dialogs.saveProgressCounter >= 100 and bfsuite.tasks.msp.mspQueue:isProcessed() then
            ...
            if app.dialogs.save ~= nil then
                ...
                if bfsuite.preferences.reloadOnSave == true then app.triggers.triggerReloadNoPrompt = true end
    ...
    if bfsuite.preferences.profileSwitching == true and app.Page ~= nil and (app.Page.refreshOnProfileChange == true or app.Page.refreshOnRateChange == true or app.Page.refreshFullOnProfileChange == true or app.Page.refreshFullOnRateChange == true) and app.uiState == app.uiStatus.pages and app.triggers.isSaving == false and bfsuite.app.dialogs.saveDisplay ~= true and bfsuite.app.dialogs.progressDisplay ~= true and bfsuite.tasks.msp.mspQueue:isProcessed() then
        ...
        if (now - app.profileCheckScheduler) >= profileCheckInterval then
            ...
            if bfsuite.session.activeProfile ~= nil and bfsuite.session.activeProfileLast ~= nil then

                if app.Page.refreshOnProfileChange == true or  app.Page.refreshFullOnProfileChange == true then
                    if bfsuite.session.activeProfile ~= bfsuite.session.activeProfileLast and bfsuite.session.activeProfileLast ~= nil then
            ...
            if bfsuite.session.activeRateProfile ~= nil and bfsuite.session.activeRateProfileLast ~= nil then

                if app.Page.refreshOnRateChange == true or app.Page.refreshFullOnRateChange == true then
                    if bfsuite.session.activeRateProfile ~= bfsuite.session.activeRateProfileLast and bfsuite.session.activeRateProfileLast ~= nil then
    ...
    if (app.dialogs.nolinkDisplay == true) and app.triggers.disableRssiTimeout == false then
        ...
        if app.dialogs.nolinkValueCounter >= 101 then
            ...
            if app.guiIsRunning == true and app.triggers.invalidConnectionSetup ~= true and app.triggers.wasConnected == false then
                ...
                local buttons = {{
                    label = bfsuite.i18n.get("app.btn_ok"),
                    action = function()
                ...
                local message
                local apiVersionAsString = tostring(bfsuite.session.apiVersion)
                ...
                if not bfsuite.utils.ethosVersionAtLeast() then
                    message = string.format(string.upper(bfsuite.i18n.get("ethos")).. " < V%d.%d.%d", 
                    bfsuite.config.ethosVersion[1], 
                    bfsuite.config.ethosVersion[2], 
                    bfsuite.config.ethosVersion[3])
                    app.triggers.invalidConnectionSetup = true
                elseif not bfsuite.tasks.active() then
                    message = bfsuite.i18n.get("app.check_bg_task") 
                    app.triggers.invalidConnectionSetup = true
                elseif  moduleState == false and app.offlineMode == false then
                    message = bfsuite.i18n.get("app.check_rf_module_on") 
                    app.triggers.invalidConnectionSetup = true 
                elseif not (sportSensor or elrsSensor)  and app.offlineMode == false then
                    message = bfsuite.i18n.get("app.check_discovered_sensors")
                    app.triggers.invalidConnectionSetup = true                                            
                elseif app.getRSSI() == 0 and app.offlineMode == false then
                    message =  bfsuite.i18n.get("app.check_heli_on")
                    app.triggers.invalidConnectionSetup = true
                elseif bfsuite.session.apiVersion == nil and app.offlineMode == false then
                    message = bfsuite.i18n.get("app.check_msp_version")
                    app.triggers.invalidConnectionSetup = true
                elseif not bfsuite.utils.stringInArray(bfsuite.config.supportedMspApiVersion, apiVersionAsString) and app.offlineMode == false then
                    message = bfsuite.i18n.get("app.check_supported_version") .. " (" .. bfsuite.session.apiVersion .. ")."
                    app.triggers.invalidConnectionSetup = true
                end
                ...
                if app.triggers.invalidConnectionSetup == true and app.triggers.wasConnected == false then
                    ...
                    form.openDialog({
                        width = nil,
                        title = bfsuite.i18n.get("error"):gsub("^%l", string.upper),
                        message = message,
                        buttons = buttons,
                        wakeup = function()
                        end,
    ...
    if bfsuite.preferences.watchdogParam ~= nil and bfsuite.preferences.watchdogParam ~= 1 then app.protocol.saveTimeout = bfsuite.preferences.watchdogParam end
    if app.dialogs.saveDisplay == true then
        if app.dialogs.saveWatchDog ~= nil then
            if (os.clock() - app.dialogs.saveWatchDog) > (tonumber(app.protocol.saveTimeout + 5)) or (app.dialogs.saveProgressCounter > 120 and bfsuite.tasks.msp.mspQueue:isProcessed()) then
                app.audio.playTimeout = true
                app.ui.progressDisplaySaveMessage(bfsuite.i18n.get("app.error_timed_out"))
    ...
    if app.dialogs.progressDisplay == true and app.dialogs.progressWatchDog ~= nil then
        ...
        app.dialogs.progressCounter = app.dialogs.progressCounter + (bfsuite.app.Page.progressCounter or 1.5)
        app.ui.progressDisplayValue(app.dialogs.progressCounter)

        if (os.clock() - app.dialogs.progressWatchDog) > (tonumber(bfsuite.tasks.msp.protocol.pageReqTimeout)) then

            app.audio.playTimeout = true

            if app.dialogs.progress ~= nil then
                app.ui.progressDisplayMessage(bfsuite.i18n.get("app.error_timed_out"))
    ...
    if app.triggers.triggerSave == true then
        local buttons = {{
            ...
            label = bfsuite.i18n.get("app.btn_cancel"),
            action = function()
                app.triggers.triggerSave = false
                return true
            end
        }}
        local theTitle = bfsuite.i18n.get("app.msg_save_settings")
        local theMsg
        if bfsuite.app.Page.extraMsgOnSave then
            theMsg = bfsuite.i18n.get("app.msg_save_current_page") .. "\n\n" .. bfsuite.app.Page.extraMsgOnSave
        else    
            theMsg = bfsuite.i18n.get("app.msg_save_current_page")
        end
    ...
    if app.triggers.triggerReload == true then
        ...
        form.openDialog({
            width = nil,
            title = bfsuite.i18n.get("reload"):gsub("^%l", string.upper),
            message = bfsuite.i18n.get("app.msg_reload_settings"),
            buttons = buttons,
            wakeup = function()
            end,
            paint = function()
   ...
   if app.triggers.triggerReloadFull == true then
    ...
    if app.triggers.isSaving then
        ...
        if app.pageState >= app.pageStatus.saving then
            ...
            if app.pageState == app.pageStatus.saving then
                app.ui.progressDisplaySaveValue(app.dialogs.saveProgressCounter, bfsuite.i18n.get("app.msg_saving_settings"))
            elseif app.pageState == app.pageStatus.eepromWrite then
                app.ui.progressDisplaySaveValue(app.dialogs.saveProgressCounter, bfsuite.i18n.get("app.msg_saving_settings"))
            elseif app.pageState == app.pageStatus.rebooting then
                app.ui.progressDisplaySaveValue(app.dialogs.saveProgressCounter, bfsuite.i18n.get("app.msg_rebooting"))
            end
    ...
    if bfsuite.preferences.saveWhenArmedWarning == true then
        if app.triggers.showSaveArmedWarning == true and app.triggers.closeSave == false then
            if app.dialogs.progressDisplay == false then
                ...
                if bfsuite.session.apiVersion >= 12.08 then
                    app.ui.progressDisplay(bfsuite.i18n.get("app.msg_save_not_commited"), bfsuite.i18n.get("app.msg_please_disarm_to_save_warning"))
                else    
                    app.ui.progressDisplay(bfsuite.i18n.get("app.msg_save_not_commited"), bfsuite.i18n.get("app.msg_please_disarm_to_save"))
                end    
            end
    ...
    if app.uiState == app.uiStatus.mainMenu then
        invalidatePages()
    elseif app.triggers.isReady and bfsuite.tasks.msp.mspQueue:isProcessed() and app.Page and app.Page.values then
    ...
    -- play audio
    -- alerts 
    if bfsuite.preferences.audioAlerts == 0 or bfsuite.preferences.audioAlerts == 1 then
        ...
        if app.audio.playSaving == true and bfsuite.preferences.audioAlerts == 0 then
            bfsuite.utils.playFile("app", "saving.wav")
            app.audio.playSaving = false
        end

        if app.audio.playLoading == true and bfsuite.preferences.audioAlerts == 0 then
...
function app.create_logtool()
    ...
    bfsuite.session.lcdWidth, bfsuite.session.lcdHeight = bfsuite.utils.getWindowSize()
    app.radio = assert(loadfile("app/radios.lua"))().msp

    app.uiState = app.uiStatus.init

    -- override developermode if file exists.
    if not bfsuite.config.developerMode and bfsuite.utils.file_exists("../developermode") then
    ...
    bfsuite.app.offlineMode = true
    bfsuite.app.ui.openPage(1, "Logs", "logs/logs.lua", 1) -- final param says to load in standalone mode
end
...
function app.event(widget, category, value, x, y)
    ...
    if value == KEY_RTN_LONG then
        bfsuite.utils.log("KEY_RTN_LONG", "info")
        invalidatePages()
        system.exit()
        return 0
    end
    ...
    if app.Page and (app.uiState == app.uiStatus.pages or app.uiState == app.uiStatus.mainMenu) then
        if app.Page.event then
            bfsuite.utils.log("USING PAGES EVENTS", "debug")
    ...
    if app.uiState == app.uiStatus.pages then
        ...
        if category == EVT_CLOSE and value == 0 or value == 35 then
            bfsuite.utils.log("EVT_CLOSE", "info")
        ...
        if value == KEY_ENTER_LONG then
            bfsuite.utils.log("EVT_ENTER_LONG (PAGES)", "info")
            if app.dialogs.progressDisplay then app.ui.progressDisplayClose() end
            if app.dialogs.saveDisplay then app.ui.progressDisplaySaveClose() end
    ...
    if app.uiState == app.uiStatus.mainMenu and value == KEY_ENTER_LONG then
         bfsuite.utils.log("EVT_ENTER_LONG (MAIN MENU)", "info")
         if app.dialogs.progressDisplay then app.ui.progressDisplayClose() end
         if app.dialogs.saveDisplay then app.ui.progressDisplaySaveClose() end
         system.killEvents(KEY_ENTER_BREAK)
         return true
