...
local graphPos = {}
graphPos['menu_offset'] = bfsuite.app.radio.logGraphMenuOffset
graphPos['height_offset'] = bfsuite.app.radio.logGraphHeightOffset or 0
graphPos['x_start'] = 0
graphPos['y_start'] = 0 + graphPos['menu_offset']
graphPos['width'] = math.floor(LCD_W * bfsuite.app.radio.logGraphWidthPercentage)
...
local logColumns = bfsuite.tasks.logging.getLogTable()
...
function readNextChunk()
    if logDataRawReadComplete then
        bfsuite.tasks.clearCallback(readNextChunk)
        return
    end
    ...
    if chunk then
        ...
        bfsuite.utils.log("Read " .. #chunk .. " bytes from log file","debug")
    else
        logFileHandle:close()
        logFileHandle = nil
        ...
        bfsuite.utils.log("Read complete, total size: " .. #logDataRaw .. " bytes","debug")
        bfsuite.tasks.clearCallback(readNextChunk)
    end
end
...
local function getLogDir()
    ...
    os.mkdir("LOGS:/bfsuite/telemetry")

    return "LOGS:/bfsuite/telemetry/"
...
local function drawKey(name, keyunit, keyminmax, keyfloor, color, minimum, maximum, laneY, laneHeight)
    ...
    lcd.font(bfsuite.app.radio.logKeyFont)
    ...
    lcd.font(bfsuite.app.radio.logKeyFontSmall)
    ...
    if bfsuite.app.radio.logShowAvg == true then
...
local function drawCurrentIndex(points, position, totalPoints, keyindex, keyunit, keyfloor, name, color, laneY, laneHeight, laneNumber, totalLanes)
    ...
    local sliderPadding = bfsuite.app.radio.logSliderPaddingLeft
...
local function openPage(pidx, title, script, logfile, displaymode)
    ...
    bfsuite.app.ui.fieldHeader("Logs - " .. extractShortTimestamp(logfile))
    ...
    bfsuite.app.formFields[1] = form.addSliderField(nil, posField, 0, 100, function()
    ...
    bfsuite.app.formFields[2] = form.addButton(line, posField, {
        text = "-",
        icon = nil,
        options = FONT_STD,
        press = function()
    ...
    bfsuite.app.formFields[3] = form.addButton(line, posField, {
        text = "+",
        icon = nil,
        options = FONT_STD,
        press = function()
            ...
            if zoomLevel == zoomCount then
                ...
                bfsuite.app.formFields[2]:focus()   
            end    
        end
    })
    
    bfsuite.app.formFields[1]:step(1)
    ...
    bfsuite.tasks.callbackEvery(0.05, readNextChunk)
...
local function onNavMenu(self)
    ...
    if currentDisplayMode == 1 then
        bfsuite.app.ui.openPage(1, bfsuite.app.lastTitle, "logs/logs.lua", 1)
    else
        bfsuite.app.ui.openPage(bfsuite.app.lastIdx, bfsuite.app.lastTitle, "logs/logs.lua")
    end
