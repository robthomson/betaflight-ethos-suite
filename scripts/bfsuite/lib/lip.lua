...
function LIP.load(fileName)
    ...
    if not bfsuite.utils.file_exists(fileName) then
        bfsuite.utils.log("LIP: Unable to find file: " .. fileName)
        return nil
    end

    local file, err = io.open(fileName, 'r')
    if not file then
        bfsuite.utils.log("LIP: Failed to open file: " .. err)
        return nil
    end
        ...
        elseif line:match("^%[.+%]$") then
            ...
            if param and value then
                ...
                if section then
                    ...
                    bfsuite.utils.log("LIP: Warning - Key-value pair found outside section in " .. fileName)
                end
            end
        end
    end
...
function LIP.save(fileName, data)
    ...
    if not file then
        bfsuite.utils.log("LIP: Failed to open file for writing: " .. err)
        return false
    end
    ...
    file:close()
    bfsuite.utils.log("LIP: Saved preferences to: " .. fileName)
    return true
end
