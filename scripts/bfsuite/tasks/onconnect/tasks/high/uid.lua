...
function uid.wakeup()
    ...
    if (bfsuite.session.mcu_id == nil and mspCallMade == false) then

        mspCallMade = true

        local API = bfsuite.tasks.msp.api.load("UID")
        API.setCompleteHandler(function(self, buf)
            ...
            if U_ID_0 and U_ID_1 and U_ID_2 then
                ...
                if uid then bfsuite.utils.log("MCU ID: " .. uid, "info") end
                bfsuite.session.mcu_id = uid
            end
...
function uid.reset()
    bfsuite.session.mcu_id = nil
    mspCallMade = false
end

function uid.isComplete() if bfsuite.session.mcu_id ~= nil then return true end end
