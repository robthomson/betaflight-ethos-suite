...
function performance.wakeup()
    ...
    if (t_now - last_mem_t) >= MEM_PERIOD then
        ...
        bfsuite.performance.freeram = mem_avg_kb
        ...
        bfsuite.performance.usedram = usedram_avg_kb

        if free_bmp_kb > bitmap_pool_est_kb then bitmap_pool_est_kb = free_bmp_kb end
        bfsuite.performance.luaBitmapsRamKB = free_bmp_kb

        bfsuite.performance.mainStackKB = (m.mainStackAvailable or 0) / 1024
        bfsuite.performance.ramKB = (m.ramAvailable or 0) / 1024
        bfsuite.performance.luaRamKB = (m.luaRamAvailable or 0) / 1024
        bfsuite.performance.luaBitmapsRamKB = (m.luaBitmapsRamAvailable or 0) / 1024
    end

    bfsuite.performance = bfsuite.performance or {}
    local loop_ms = tonumber(bfsuite.performance.taskLoopCpuMs) or tonumber(bfsuite.performance.taskLoopTime) or 0
    ...
    if win_t >= 0.10 then
        ...
        bfsuite.performance.cpuload_window100 = window_util * 100
    ...
    bfsuite.performance.cpuload = clamp(cpu_avg * 100, 0, 100)

    bfsuite.performance.loop_ms = loop_ms
    bfsuite.performance.budget_ms = budget_ms
    bfsuite.performance.util_raw = instant_util * 100
    bfsuite.performance.tick_ms = dt * 1000.0
