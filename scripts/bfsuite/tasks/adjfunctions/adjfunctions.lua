...
function adjfunc.wakeup()
    ...
    if bfsuite.preferences.adjFunctionAlerts == false and bfsuite.preferences.adjValueAlerts == false then return end


    if (os.clock() - initTime) < 5  then return end

    -- getSensor source has a cache built in - win
    adjfuncAdjFunctionSrc = bfsuite.tasks.telemetry.getSensorSource("adj_f")
    adjfuncAdjValueSrc = bfsuite.tasks.telemetry.getSensorSource("adj_v")
            ...
            if adjfuncAdjFunction ~= 0 then
                ...
                if (os.clock() - adjfuncAdjTimer >= 2) then
                    if adjfuncAdjfuncIdChanged == true then
                        ...
                        if adjfunction ~= nil and firstRun == false then for wavi, wavv in ipairs(adjfunction.wavs) do if bfsuite.preferences.adjFunctionAlerts == true then bfsuite.utils.playFile("adjfunctions", wavv .. ".wav") end end end
                        adjfuncAdjfuncIdChanged = false
                    end
                    if adjfuncAdjfuncValueChanged == true or adjfuncAdjfuncIdChanged == true then

                        if adjfuncAdjValue ~= nil and firstRun == false then if bfsuite.preferences.adjValueAlerts == true then system.playNumber(adjfuncAdjValue) end end
