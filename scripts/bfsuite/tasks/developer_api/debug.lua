...
function apidebug.wakeup()
    ...
    -- This is an example of how to use the api library to query the governor mode
        bfsuite.utils.log("API Debug Task: GOVERNOR_CONFIG", "info")
    local API = bfsuite.tasks.msp.api.load("GOVERNOR_CONFIG")
    API.setCompleteHandler(function(self, buf)
        local governorMode = API.readValue("gov_mode")
        bfsuite.utils.log("Governor mode: " .. governorMode, "info")
        bfsuite.session.governorMode = governorMode
    end)
    API.setUUID("123e4567-e89b-12d3-a456-426614174000")
    API.read()
    ]]--

    bfsuite.utils.log("API Debug Task: TELEMETRY_CONFIG", "info")
    local API = bfsuite.tasks.msp.api.load("TELEMETRY_CONFIG")
    API.setCompleteHandler(function(self, buf)
    end)
    API.setUUID("123e4567-e89b-12d3-a456-426614174000")
    API.read()
