...
--[[
    ...
    This function checks if the bfsuite tasks are active and if the API version is available. 
...
local function loadSensorModule()
    if not bfsuite.tasks.active() then return nil end
    if not bfsuite.session.apiVersion then return nil end

    local protocol = bfsuite.tasks.msp.protocol.mspProtocol

    if system:getVersion().simulation == true and bfsuite.preferences.internalSimSensors == true then
        if not loadedSensorModule or loadedSensorModule.name ~= "sim" then
            --bfsuite.utils.log("Loading Simulator sensor module","info")
            loadedSensorModule = {name = "sim", module = assert(loadfile("tasks/sensors/sim.lua"))(config)}
        end   
    elseif protocol == "crsf" and bfsuite.preferences.internalElrsSensors then
        if not loadedSensorModule or loadedSensorModule.name ~= "elrs" then
            --bfsuite.utils.log("Loading ELRS sensor module","info")
            loadedSensorModule = {name = "elrs", module = assert(loadfile("tasks/sensors/elrs.lua"))(config)}
        end
    elseif protocol == "sport" and bfsuite.preferences.internalSportSensors then
        if bfsuite.utils.round(bfsuite.session.apiVersion,2) >= 12.08 then
            if not loadedSensorModule or loadedSensorModule.name ~= "frsky" then
                --bfsuite.utils.log("Loading FrSky sensor module","info")
            ...
            if not loadedSensorModule or loadedSensorModule.name ~= "frsky_legacy" then
                --bfsuite.utils.log("Loading FrSky Legacy sensor module","info")
...
function sensors.wakeup()
    ...
    if bfsuite.session.resetSensors and not delayPending then
        delayStartTime = os.clock()
        delayPending = true
        bfsuite.session.resetSensors = false  -- Reset immediately
        bfsuite.utils.log("Delaying sensor wakeup for " .. delayDuration .. " seconds","info")
        return  -- Exit early; wait starts now
    end

    if delayPending then
        if os.clock() - delayStartTime >= delayDuration then
            bfsuite.utils.log("Delay complete; resuming sensor wakeup","info")
            delayPending = false
        else
