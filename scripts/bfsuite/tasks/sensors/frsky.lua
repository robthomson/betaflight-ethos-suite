    ...
    if createSensorList[appId] ~= nil then
        ...
        if frsky.createSensorCache[appId] == nil then
            ...
            if frsky.createSensorCache[appId] == nil then
                ...
                bfsuite.utils.log("Creating sensor: " .. v.name, "info")
                ...
                frsky.createSensorCache[appId]:module(bfsuite.session.telemetrySensor:module())
...
local function dropSensor(physId, primId, appId, frameValue)
    ...
    if bfsuite.session.apiVersion >= 12.08 then return end
    ...
    if dropSensorList[appId] ~= nil then
        ...
        if frsky.dropSensorCache[appId] == nil then
            ...
            if frsky.dropSensorCache[appId] ~= nil then
                bfsuite.utils.log("Drop sensor: " .. v.name, "info")
        ...
        if frsky.renameSensorCache[appId] == nil then
            ...
            if frsky.renameSensorCache[appId] ~= nil then
                if frsky.renameSensorCache[appId]:name() == v.onlyifname then
                    bfsuite.utils.log("Rename sensor: " .. v.name, "info")
...
local function telemetryPop()
    local frame = bfsuite.tasks.msp.sensorTlm:popFrame()
...
function frsky.wakeup()
    ...
    if not bfsuite.session.telemetryState or not bfsuite.session.telemetrySensor then clearCaches() end

    -- If GUI or queue is busy.. do not do this!
    if bfsuite.tasks and bfsuite.tasks.telemetry and bfsuite.session.telemetryState and bfsuite.session.telemetrySensor then if bfsuite.app.guiIsRunning == false and bfsuite.tasks.msp.mspQueue:isProcessed() then while telemetryPop() do end end end
