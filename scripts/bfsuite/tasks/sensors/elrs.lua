...
--
-- Betaflight Custom Telemetry Decoder for ELRS
--
--
local arg = {...}
local config = arg[1]
...
local function createTelemetrySensor(uid, name, unit, dec, value, min, max)
    ...
    if bfsuite.session.telemetryState == false then return end
...
local function setTelemetryValue(uid, subid, instance, value, unit, dec, name, min, max)
    ...
    if sensors['uid'][uid] == nil then
        ...
        if sensors['uid'][uid] == nil then
            bfsuite.utils.log("Create sensor: " .. uid, "debug")
...
function elrs.crossfirePop()
    ...
    if (CRSF_PAUSE_TELEMETRY == true or bfsuite.app.triggers.mspBusy == true or bfsuite.session.telemetryState == false) then
        ...
        if bfsuite.session.telemetryState == false then
            sensors['uid'] = {}
            sensors['lastvalue'] = {}
        end
...
function elrs.wakeup()
    ...
    if bfsuite.session.telemetryState and bfsuite.session.telemetrySensor then
        while elrs.crossfirePop() do
            if CRSF_PAUSE_TELEMETRY == true or bfsuite.app.triggers.mspBusy == true  then
                break
            end
        end
    else
