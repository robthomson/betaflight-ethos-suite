...
local function generateLogFilename()
    local craftName = bfsuite.utils.sanitize_filename(bfsuite.session.craftName)
...
function logging.writeLogs(forcewrite)
    ...
    if #log_queue > 0 and logFileName then
        bfsuite.utils.log("Write " .. #log_queue .. " lines to " .. logFileName,"info")
        local filePath = "LOGS:bfsuite/telemetry/" .. logFileName
...
function logging.getLogHeader()
    ...
    return "time, " .. bfsuite.utils.joinTableItems(names, ", ")
end
...
function logging.getLogLine()
    ...
    return os.date("%Y-%m-%d_%H:%M:%S") .. ", " .. bfsuite.utils.joinTableItems(values, ", ")
end
...
local function cacheSensorSources()
    ...
    for _, sensor in ipairs(logTable) do
        cachedSensors[sensor.name] = bfsuite.tasks.telemetry.getSensorSource(sensor.name)
    end
    armSource = bfsuite.tasks.telemetry.getSensorSource("armflags")
    govSource = bfsuite.tasks.telemetry.getSensorSource("governor")
end
...
function logging.flushLogs()
    if logFileName or logHeader then
        bfsuite.utils.log("Flushing logs - ".. logFileName,"debug")
...
function logging.wakeup()
    if not bfsuite.preferences.flightLog then return end
    ...
    if armSource then
        ...
        if armSource and not govSource then
            ...
            if isArmed == 1 or isArmed == 3 then
                if not logFileName then 
                    ...
                    bfsuite.utils.log("Logging triggered by arm state - " .. logFileName,"debug")
                end
        ...
        elseif armSource and govSource then
            ...
            if isArmed == 1 or isArmed == 3 and governor > 0 and governor < 100 then
                if not logFileName then 
                    logFileName = generateLogFilename() 
                    bfsuite.utils.log("Logging triggered by governor state - " .. logFileName,"debug")
                end
                if not logHeader then
                    logHeader = logging.getLogHeader()
                    logging.queueLog(logHeader)
