...
--[[
    ...
    Dependencies:
        bfsuite.utils.log (function): Logs messages for debugging purposes.
        bfsuite.tasks.msp.protocol.maxTxBufferSize (number): The maximum size of the transmission buffer.
        bfsuite.tasks.msp.protocol.mspSend (function): Sends the MSP payload.
...
function mspProcessTxQ()
    ...
    bfsuite.utils.log("Sending mspTxBuf size " .. tostring(#mspTxBuf) .. " at Idx " .. tostring(mspTxIdx) .. " for cmd: " .. tostring(mspLastReq),"debug")
...
function mspSendRequest(cmd, payload)
    if not cmd or type(payload) ~= "table" then
        bfsuite.utils.log("Invalid command or payload","debug")
        return nil
    end
    if #mspTxBuf ~= 0 then
        bfsuite.utils.log("Existing mspTxBuf still sending, failed to send cmd: " .. tostring(cmd),"debug")
        return nil
    end
    mspTxBuf[1] = #payload
    mspTxBuf[2] = cmd & 0xFF
...
local function mspReceivedReply(payload)
    ...
    while (idx <= bfsuite.tasks.msp.protocol.maxRxBufferSize) and (#mspRxBuf < mspRxSize) do
        ...
        local value = tonumber(payload[idx])  -- or use string.byte(payload[idx]) if payload[idx] is a string
        if value then
            mspRxCRC = mspRxCRC ~ value
        else
            bfsuite.utils.log("Non-numeric value at payload index " .. idx,"debug")
        end
        idx = idx + 1
    end
    ...
    if mspRxCRC ~= payload[idx] and version == 0 then
        bfsuite.utils.log("Payload checksum incorrect, message failed!","debug")
        return nil
    end
    return true
end
