...
local function setLogging(level, opts)
    ...
    bfsuite.utils.log(_mspPrefix .. "logging set to level=" .. tostring(_mspLogLevel) .. (_mspHexDump and " (hexdump on)" or " (hexdump off)"), "debug")
end

local function getLogging() return {level = _mspLogLevel, hexdump = _mspHexDump, prefix = _mspPrefix} end

local function _log(level, msg) if _mspLogLevel >= level then bfsuite.utils.log(_mspPrefix .. msg, "info") end end
...
local function _hex(buf, maxLen)
    ...
    bfsuite.utils.log(_mspPrefix .. "HEX[" .. tostring(n) .. "]: " .. table.concat(t, " "), "debug")
    if #buf > n then bfsuite.utils.log(_mspPrefix .. "(+" .. tostring(#buf - n) .. " more bytes)", "debug") end
end
...
function handlers.v1.processTxQ()
    ...
    if i <= bfsuite.tasks.msp.protocol.maxTxBufferSize then
        ...
        _hex(payload, bfsuite.tasks.msp.protocol.maxTxBufferSize)
...
function handlers.v1.sendRequest(cmd, payload)
    ...
    _hex(mspTxBuf, bfsuite.tasks.msp.protocol.maxTxBufferSize)
end
...
function handlers.v1.receivedReply(payload)
    ...
    _hex(mspRxBuf, bfsuite.tasks.msp.protocol.maxRxBufferSize)
...
function handlers.v2.processTxQ()
    ...
    for j = i, bfsuite.tasks.msp.protocol.maxTxBufferSize do payload[j] = payload[j] or 0 end
