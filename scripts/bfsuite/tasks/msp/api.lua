...
local apidir = "SCRIPTS:/".. bfsuite.config.baseDir  .. "/tasks/msp/api/"
local api_path = apidir

-- New version using the global callback system
function apiLoader.scheduleWakeup(func)
    if bfsuite and bfsuite.tasks and bfsuite.tasks.callbackNow then
        bfsuite.tasks.callbackNow(func)
    else
        bfsuite.utils.log("ERROR: bfsuite.tasks.callbackNow() is missing!", "error")
    end
end
...
--[[
    ...
    2. Checks if the file exists using `bfsuite.utils.file_exists`.
...
local function loadAPI(apiName)
    ...
    if bfsuite.utils.file_exists(apiFilePath) then
        ...
        if type(apiModule) == "table" and (apiModule.read or apiModule.write) then
            ...
            bfsuite.utils.log("Loaded API: " .. apiName, "debug")
            return apiModule
        else
            bfsuite.utils.log("Error: API file '" .. apiName .. "' does not contain valid read or write functions.", "debug")
        end
    else
        bfsuite.utils.log("Error: API file '" .. apiFilePath .. "' not found.", "debug")
    end
end
...
function apiLoader.load(apiName)
    ...
    if api == nil then
        bfsuite.utils.log("Unable to load " .. apiName,"debug")
    end
    return api
end
...
local function parseMSPChunk(buf, structure, state)
    local apiVersion = bfsuite.session.apiVersion or 12.06
    ...
    while state.index <= #structure and processedFields < 5 do  -- Process 5 fields per wakeup
        ...
        local readFunction = bfsuite.tasks.msp.mspHelper["read" .. field.type]
        if not readFunction then
            bfsuite.utils.log("Error: No reader for type: " .. field.type, "debug")
    ...
    bfsuite.utils.log(string.format("Chunk processed - fields %d to %d", startIndex, state.index - 1), "debug")
...
function apiLoader.parseMSPData(buf, structure, processed, other, options)
    ...
    if chunked then
        ...
        local function processNextChunk()
            ...
            bfsuite.utils.log(string.format("Chunk processed - fields %d to %d", 
            ...
            if state.index > #structure then
                if completionCallback and buf.offset then
                    ...
                    bfsuite.utils.log("Completion callback not provided or buffer offset not available", "info")
                    completionCallback({
                        parsed = {state.parsedData},
                        buffer = {},
                        structure = structure,
        ...
        if buf.offset <= #buf then
            ...
            bfsuite.utils.log("Unused bytes in buffer (" .. extraBytes .. " extra bytes)", "debug")
        elseif buf.offset > #buf + 1 then
            bfsuite.utils.log("Offset exceeded buffer length (Offset: " .. buf.offset .. ", Buffer: " .. #buf .. ")", "debug")
        end
...
function apiLoader.filterByApiVersion(structure)
    ...
    for _, param in ipairs(structure) do
        ...
        -- API version check logic
        if not param.apiVersion or (apiVersion and bfsuite.utils.round(apiVersion,2) >= bfsuite.utils.round(param.apiVersion,2)) then
            insert_param = true
        end
...
function apiLoader.buildWritePayload(apiname, payload, api_structure, noDelta)
    local positionmap = bfsuite.app.Page.mspapi and bfsuite.app.Page.mspapi.positionmap[apiname]
    local receivedBytes = bfsuite.app.Page.mspapi and bfsuite.app.Page.mspapi.receivedBytes[apiname]
    local receivedBytesCount = bfsuite.app.Page.mspapi and bfsuite.app.Page.mspapi.receivedBytesCount[apiname]
    ...
    if useDelta then
        bfsuite.utils.log("[buildWritePayload] Using delta updates for " .. apiname, "info")
        return apiLoader.buildDeltaPayload(apiname, payload, api_structure, positionmap, receivedBytes, receivedBytesCount)
    else
        bfsuite.utils.log("[buildWritePayload] Using full rebuild for " .. apiname, "info")
...
function apiLoader.buildDeltaPayload(apiname, payload, api_structure, positionmap, receivedBytes, receivedBytesCount)
    ...
    for idx, formField in ipairs(bfsuite.app.formFields) do
        local pageField = bfsuite.app.Page.fields[idx]
    ...
    local actual_fields = {}
    if bfsuite.app.Page and bfsuite.app.Page.mspapi then
        for _, field in ipairs(bfsuite.app.Page.mspapi.formdata.fields) do
    ...
    for _, field_def in ipairs(api_structure) do
        ...
        if not editableFields[field_name] then
            bfsuite.utils.log("[buildDeltaPayload] Skipping non-editable field: " .. field_name, "debug")
            goto continue
        end
        ...
        local writeFunction = bfsuite.tasks.msp.mspHelper["write" .. field_def.type]
        ...
        if positionmap[field_name] then
            ...
            bfsuite.utils.log(string.format(
...
function apiLoader.buildFullPayload(apiname, payload, api_structure)
    ...
    bfsuite.utils.log("[buildFullPayload] Clearing byte stream for full rebuild", "debug")
