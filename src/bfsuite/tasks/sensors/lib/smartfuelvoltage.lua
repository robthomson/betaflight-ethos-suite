...
local telemetry
local currentMode = bfsuite.flightmode.current or "preflight"
...
local function getStickLoadFactor()
    local rx = bfsuite.session.rx.values
...
local function applySagCompensation(voltage)
    if bfsuite.flightmode.current ~= "inflight" then return voltage end
    local multiplier = bfsuite.session.modelPreferences and bfsuite.session.modelPreferences.battery and bfsuite.session.modelPreferences.battery.sag_multiplier or 0.7
...
local function fuelPercentageCalcByVoltage(voltage, cellCount)
    local bc = bfsuite.session.batteryConfig
...
local function smartFuelCalc()
    if not telemetry then telemetry = bfsuite.tasks.telemetry end

    if not bfsuite.session.isConnected or not bfsuite.session.batteryConfig then
        resetVoltageTracking()
        return nil
    end

    if bfsuite.session.modelPreferences and bfsuite.session.modelPreferences.battery and bfsuite.session.modelPreferences.battery.calc_local then
        if lastSensorMode ~= bfsuite.session.modelPreferences.battery.calc_local then
            resetVoltageTracking()
            lastSensorMode = bfsuite.session.modelPreferences.battery.calc_local
        end
    end
    ...
    if currentMode ~= lastMode then
        bfsuite.utils.log("Flight mode changed – resetting voltage state", "info")
        resetVoltageTracking()
    ...
    if not voltageStabilised then
        if isVoltageStable() then
            bfsuite.utils.log("Voltage stabilized at: " .. voltage, "info")
            voltageStabilised = true
        else
            bfsuite.utils.log("Waiting for voltage to stabilize...", "info")
            return nil
        end
    end

    if #lastVoltages >= 1 and bfsuite.flightmode.current == "preflight" then
        local prev = lastVoltages[#lastVoltages - 1]
        if voltage > prev + voltageThreshold then
            bfsuite.utils.log("Voltage increased after stabilization – resetting...", "info")
            resetVoltageTracking()
            stabilizeNotBefore = os.clock() + preStabiliseDelay
            return nil
        end
    ...
    local now = os.clock()
    if (bfsuite.flightmode.current == "inflight" or bfsuite.flightmode.current == "postflight") and lastFuelPercent and lastFuelTimestamp then
