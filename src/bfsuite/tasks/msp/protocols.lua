...
function protocol.getProtocol()
    if bfsuite.session and bfsuite.session.telemetryType then
        if bfsuite.session.telemetryType == "crsf" then
...
function protocol.getTransports()
    ...
    for i, v in pairs(supportedProtocols) do
        transport[i] = "SCRIPTS:/" .. bfsuite.config.baseDir .. "/tasks/msp/" .. v.mspTransport
    end
    return transport
end
