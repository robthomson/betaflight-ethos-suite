...
function tasks.isTaskActive(name)
    for _, t in ipairs(tasksList) do
        if t.name == name then
            ...
            if name == "msp" then
                return bfsuite.session.mspBusy
...
local function clearSessionAndQueue()
    ...
    utils.session()
    local q = bfsuite.tasks and bfsuite.tasks.msp and bfsuite.tasks.msp.mspQueue
...
function tasks.telemetryCheckScheduler()
    ...
    if system.getVersion().simulation and bfsuite.simevent.telemetry_state == false then telemetryState = false end
    ...
    if currentSensor then
        ...
        bfsuite.session.telemetrySensor = currentSensor
        bfsuite.session.telemetryModule = currentModuleId
        bfsuite.session.telemetryType = currentTelemetryType
        bfsuite.session.telemetryModuleNumber = currentModuleNumber
    ...
    if currentTelemetryType ~= lastTelemetryType then
        bfsuite.utils.log("Telemetry type changed to " .. tostring(currentTelemetryType), "info")
        tasks.setTelemetryTypeChanged()
...
local function canRunTask(task, now)
    ...
    local linkOK = not task.linkrequired or bfsuite.session.telemetryState
    local connOK = not task.connected or bfsuite.session.isConnected

    local ok = linkOK and connOK and (priorityTask or od >= 0 or not bfsuite.session.mspBusy) and (not task.simulatoronly or usingSimulator)
...
function tasks.wakeup()
    ...
    tasks.profile.enabled = bfsuite.preferences and bfsuite.preferences.developer and bfsuite.preferences.developer.taskprofiler
    ...
    if ((bfsuite.app and bfsuite.app.guiIsRunning and not bfsuite.app.escPowerCycleLoader) or not bfsuite.session.isConnected) and bfsuite.session.mspBusy then
        if cycleFlip == 0 then
            if tasks.msp then
    ...
    local t1 = os.clock()
    bfsuite.performance = bfsuite.performance or {}
    bfsuite.performance.taskLoopCpuMs = loopCpu * 1000.0
    bfsuite.performance.taskLoopTime = (t1 - t0) * 1000.0
end
...
function tasks.load(name, meta)
    ...
    if not meta then
        local initPath = "SCRIPTS:/" .. bfsuite.config.baseDir .. "/tasks/" .. name .. "/init.lua"
    ...
    local scriptPath = "SCRIPTS:/" .. bfsuite.config.baseDir .. "/tasks/" .. name .. "/" .. meta.script
    local fn, loadErr = compiler(scriptPath)
    if not fn then
