...
local function openPage(pageIdx, title, script)
    ...
    bfsuite.app.ui.fieldHeader("@i18n(app.modules.settings.name)@" .. " / " .. "@i18n(app.modules.settings.triggers)@")
    ...
    bfsuite.app.formLines[formLineCnt] = form.addLine("@i18n(app.modules.settings.model_armswitch)@")
    ...
    formFieldCount = formFieldCount + 1
    formLineCnt = formLineCnt + 1
    bfsuite.app.formLines[formLineCnt] = form.addLine("@i18n(app.modules.settings.model_inflightswitch)@")
    bfsuite.app.formFields[formFieldCount] = form.addSwitchField(bfsuite.app.formLines[formLineCnt], nil, function()
    ...
    formFieldCount = formFieldCount + 1
    formLineCnt = formLineCnt + 1
    bfsuite.app.formLines[formLineCnt] = form.addLine("@i18n(app.modules.settings.model_inflightswitch_delay)@")
...
local function onSaveMenu()
    ...
    form.openDialog({width = nil, title = "@i18n(app.modules.profile_select.save_dashx.preferences.model)@", message = "@i18n(app.modules.profile_select.save_prompt_local)@", buttons = buttons, wakeup = function() end, paint = function() end, options = TEXT_LEFT})
end

local function event(widget, category, value, x, y)

    if (category == EVT_CLOSE and value == 0) or value == 35 then
        bfsuite.app.ui.openPage(pageIdx, "@i18n(app.modules.settings.name)@", "model/model.lua")
        return true
    end
end
