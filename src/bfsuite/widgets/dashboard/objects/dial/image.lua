...
bfsuite.session.dialImageCache = bfsuite.session.dialImageCache or {}
local function loadDialPanelCached(dialId)
    local key = tostring(dialId or "panel1")
    if not bfsuite.session.dialImageCache[key] then
        local panelPath = resolveDialAsset(dialId, "widgets/dashboard/gfx/dials") or "widgets/dashboard/gfx/dials/panel1.png"
        bfsuite.session.dialImageCache[key] = bfsuite.utils.loadImage(panelPath)
    end
    return bfsuite.session.dialImageCache[key]
end
